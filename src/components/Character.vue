<template>
  <div class="flex items-center bg-gray-100 border border-black/10 rounded-md shadow-sm flex-row dark:border-gray-300/20 dark:bg-grey">
    ...
    <div class="flex-1 flex-col justify-between pl-2 pr-2 leading-normal">
      <a :href="character.link" target="_blank" class="font-normal text-[0.95rem] hover:underline text-gray-700 dark:text-gray-400">{{ character.name }}</a>
      <p class="font-normal text-xs mt-1 text-gray-700 dark:text-gray-400">{{ character.role }}</p>
    </div>

    <div v-if="!isStaff && character.seiyuu.name.length > 0" class="flex-1 flex-col justify-between pl-6 pr-2 leading-normal">
      <p @click="redirect(character.seiyuu.link)" class="font-normal hover:cursor-pointer text-[0.95rem] hover:underline text-gray-700 dark:text-gray-400 text-end">{{ character.seiyuu.name }}</p>
      <p class="font-normal text-xs mt-1 text-gray-700 dark:text-gray-400 text-end">Japanese</p>
    </div>
